...
class TaskSampler(Sampler):
    ...
    def episodic_collate_fn(
        ...
        true_class_ids = list({x[2] for x in input_data_with_int_labels})
        ...
        # add text part
        all_texts = np.array([x[1] for x in input_data_with_int_labels])
        
        #all_texts = np.array(all_texts)
        #all_texts = all_texts.reshape((self.n_way, self.n_shot + self.n_query, *all_texts.shape[2:]))
        
        all_labels = torch.tensor([true_class_ids.index(x[2]) for x in input_data_with_int_labels]).reshape((self.n_way, self.n_shot + self.n_query))

        support_images = all_images[:, : self.n_shot].reshape((-1, *all_images.shape[2:]))
        support_texts = all_texts[: self.n_way * self.n_shot].flatten().tolist()
        support_labels = all_labels[:, : self.n_shot].flatten()
        
        query_images = all_images[:, self.n_shot :].reshape((-1, *all_images.shape[2:]))
        query_texts = all_texts[self.n_way * self.n_shot :].flatten().tolist()
    ...
    def _cast_input_data_to_tensor_int_tuple(
        ...
        """
        for image, text, label in input_data:
            ...
            #if not isinstance(text, Tensor):
            #    raise TypeError(
            #        f"Illegal type of input instance: {type(text)}. "
            #        + GENERIC_TYPING_ERROR_MESSAGE
            #    )
        ...
        return [(image, text, int(label)) for (image, text, label) in input_data]
